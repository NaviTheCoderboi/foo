        ...
        for (const option of options) {
            ...
            if (option.required) {
                ...
                if (value === true || (value === false && !hasNegated)) {
                    throw new CACError(
                        `option \`${option.rawName}\` value is missing`
                    );
                }
            }
        }
...
var buildSvelte = async (content, sp, opts) => {
    ...
    const warnings = [];
    if (sp) {
        sveltePreprocess = await $import('svelte-preprocess');
    }
    const processed = await svelte.preprocess(content, [
        {
            script: async (_opts) => {
                const { json, toStr } = modifyRawJson(opts.tsconfigRaw);
...
var loadTsConfig = async (tsconfig) => {
    const tsconfigPath = path.isAbsolute(tsconfig)
        ? tsconfig
    ...
    try {
        ...
        if (error.message === 'errors') {
            ...
            logger_default.warn(
                "Couldn't find tsconfig file, using default options"
            );
        }
        return {
            raw: '{}',
...
program
    .command('build [...entry]', 'Build the library')
    .option('-o, --outdir <output>', 'Output directory')
    .option('-f, --format <format>', 'Module format')
    .option('-w, --watch', 'Watch mode')
    .option('--tsconfig <tsconfig>', 'Path to tsconfig file')
    .option('-b, --bundle', 'Bundle the files')
    .option('--minify', 'Minify the files')
    .option('-s, --sourcemap', 'Generate sourcemaps')
    .option('-p, --platform <platform>', 'Platform to build for')
    .option('--splitting', 'Enable code splitting')
    .option('--target <...target>', 'Target environment')
    .option('--svelte-preprocess', 'Use svelte preprocess')
    .option('--out-base <outBase>', 'Base path for the output files')
    .option('-c, --clean', 'Clean the output directory before building')
    .option('--detailed-build-errors', 'Show detailed build errors')
    .option('-i, --ignore <...ignore>', 'Files to ignore')
    .option('-dts', 'Generate dts files')
    .option('--svelte', 'Whether to build .svelte files')
    .action(async (entry, opts) => {
        ...
        if (
            cliConfig.entry?.length === 0 &&
            fileConfig?.entry?.length &&
            fileConfig.entry.length > 0
        ) {
            finalConfig.entry = fileConfig.entry;
        }
        if (
            cliConfig.target?.length === 0 &&
            fileConfig?.target?.length &&
            fileConfig.target.length > 0
        ) {
            finalConfig.target = fileConfig.target;
        }
        if (
            cliConfig.ignore?.length === 0 &&
            fileConfig?.ignore?.length &&
            fileConfig.ignore.length > 0
        ) {
