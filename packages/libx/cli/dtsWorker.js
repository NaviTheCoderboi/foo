...
if (
    data.config.svelte &&
    data.config.entry.some((file) => file.endsWith('.svelte'))
) {
    svelte = await $import('svelte');
    s2x = await $import('svelte2tsx');
}
var isSvelteFile = (file) => file.endsWith('.svelte');
var isTsScript = (content) =>
    /<script\s+[^>]*?lang=('|")(ts|typescript)('|")/.test(content);
var compileSvelte = (pth, content) =>
    s2x.svelte2tsx(content, {
...
var servicesHost = {
    ...
    readFile: (pth, encoding) => {
        ...
        if (isSvelteFile(corrected)) {
            return (
                files.get(corrected)?.content ??
                ts.sys.readFile(corrected, encoding ?? 'utf-8')
            );
        }
...
servicesHost.resolveModuleNameLiterals = (
    moduleLiterals,
    containingFile,
...
servicesHost.resolveModuleNames = (
    moduleNames,
    containingFile,
    _reusedNames,
    _redirectedReference,
...
var rebuild = async (once) => {
    ...
    if (!once) {
        ...
        for (const file of files) {
            ...
            if (oldHash !== newHash) {
                files.set(file[0], {
                    content: isSvelteFile(file[0])
                        ? compileSvelte(file[0], newContent)
                        : newContent,
